...
void UART::Init(UART_HardWare hw, const UART_Config_t &cfg, uint32 pclk)
{
    ...
    if (cfg.WordLength == UART_WordLength::BITS_9)
        SET_BIT(reg->CR1, 12);
    else
        CLEAR_BIT(reg->CR1, 12);

    if (cfg.Parity == UART_Parity::NONE) {
        CLEAR_BIT(reg->CR1, 10);
    } else {
        SET_BIT(reg->CR1, 10);
        if (cfg.Parity == UART_Parity::ODD)
            SET_BIT(reg->CR1, 9);
        else
            CLEAR_BIT(reg->CR1, 9);
    }
...
{
    USART_RegDef_t *reg = UART::GetReg ? nullptr : nullptr;
    /* resolve manually */
    uint32 base;
    switch (hw)
    {
