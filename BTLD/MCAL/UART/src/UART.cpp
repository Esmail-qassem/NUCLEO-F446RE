...
{
    uint8 buf[10]; uint8 cnt = 0;
    while (num > 0) { buf[cnt++] = static_cast<uint8>((num % 10) + '0'); num /= 10; }
    for (uint8 i = cnt; i > 0u; i--) UART::SendSyncBuffer(hw, buf + i - 1, 1u);
}
...
void UART::Init(UART_HardWare hw, const UART_Config_t &cfg, uint32 pclk)
{
    ...
    reg->BRR = (pclk + (cfg.BaudRate / 2U)) / cfg.BaudRate;
    if (cfg.WordLength == UART_WordLength::BITS_9) SET_BIT(reg->CR1, 12); else CLEAR_BIT(reg->CR1, 12);
    if (cfg.Parity == UART_Parity::NONE) { CLEAR_BIT(reg->CR1, 10); }
    else { SET_BIT(reg->CR1, 10); if (cfg.Parity == UART_Parity::ODD) SET_BIT(reg->CR1, 9); else CLEAR_BIT(reg->CR1, 9); }
    reg->CR2 &= ~(0x3u << 12u);
...
void UART::SendSyncBuffer(UART_HardWare hw, const uint8 *buf, uint8 size)
{
    USART_RegDef_t *reg = GetReg(hw);
    if (!reg || !buf) return;
    for (uint8 i = 0u; i < size; i++) { while (!GET_BIT(reg->SR, 7)) {} reg->DR = buf[i]; }
}

void UART::SendNumber(UART_HardWare hw, sint32 number)
{
    if (number < 0) { uint8 m = '-'; SendSyncBuffer(hw, &m, 1u); number = -number; }
    if (number == 0) { uint8 z = '0'; SendSyncBuffer(hw, &z, 1u); return; }
...
static void UART_HandleIRQ(UART_HardWare hw, void(*cb)(uint8))
{
    ...
    if (status & (1u<<5u)) { rx_test = static_cast<uint8>(*DR_ptr); if (cb) cb(rx_test); }
    if (status & (1u<<3u)) { volatile uint32 t = *SR_ptr; t = *DR_ptr; (void)t; }
    if (status & (1u<<2u)) { volatile uint32 t = *SR_ptr; (void)t; }
    if (status & (1u<<1u)) { volatile uint32 t = *SR_ptr; (void)t; }
}
