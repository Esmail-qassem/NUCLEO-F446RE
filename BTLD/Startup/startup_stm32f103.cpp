...
void Reset_Handler(void)
{
    uint32 Section_Size = 0u;
    uint32 *MemSourceAddr = nullptr;
    uint32 *MemDestAddr   = nullptr;

    Section_Size  = static_cast<uint32>(&_edata - &_sdata);
    MemSourceAddr = reinterpret_cast<uint32*>(&_sidata);
    MemDestAddr   = reinterpret_cast<uint32*>(&_sdata);
    for (uint32 i = 0u; i < Section_Size; i++) *MemDestAddr++ = *MemSourceAddr++;

    Section_Size = static_cast<uint32>(&_ebss - &_sbss);
    MemDestAddr  = reinterpret_cast<uint32*>(&_sbss);
    for (uint32 i = 0u; i < Section_Size; i++) *MemDestAddr++ = 0u;
