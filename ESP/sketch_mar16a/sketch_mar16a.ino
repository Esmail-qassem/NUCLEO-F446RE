...
String getSTM32Version()
{
    ...
    /* Flush any garbage in RX buffer before sending command */
    ...
    dbg("Version bytes: " + String(version.length()));
    dbg("Version: [" + version + "]");
...
void loop()
{
    ...
    if (millis() - lastPollTime >= POLL_INTERVAL_MS)
    {
        ...
        {
            dbg("STM32:  [" + currentVersion + "] len=" + String(currentVersion.length()));
            dbg("Server: [" + serverVersion + "] len=" + String(serverVersion.length()));
            dbg("Equal:  " + String(currentVersion == serverVersion));
