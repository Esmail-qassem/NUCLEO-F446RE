...
NVIC_Status NVIC::EnableInterrupt(uint8 irqNum)
{
    ...
    else if (irqNum < 64u) { NVIC_ISER(1) = (1u << (irqNum - 32u)); return NVIC_Status::OK; }
...
NVIC_Status NVIC::DisableInterrupt(uint8 irqNum)
{
    ...
    else if (irqNum < 64u) { NVIC_ICER(1) = (1u << (irqNum - 32u)); return NVIC_Status::OK; }
    return NVIC_Status::NOK;
}
...
NVIC_Status NVIC::SetPendingFlag(uint8 irqNum)
{
    ...
    else if (irqNum < 64u) { NVIC_ISPR(1) = (1u << (irqNum - 32u)); return NVIC_Status::OK; }
    return NVIC_Status::NOK;
}
...
NVIC_Status NVIC::ClearPendingFlag(uint8 irqNum)
{
    ...
    else if (irqNum < 64u) { NVIC_ICPR(1) = (1u << (irqNum - 32u)); return NVIC_Status::OK; }
    return NVIC_Status::NOK;
}
...
NVIC_Status NVIC::GetActiveFlag(uint8 irqNum, uint8 &out)
{
    ...
    else if (irqNum < 64u) { out = static_cast<uint8>(GET_BIT(NVIC_IABR(1), irqNum - 32u)); return NVIC_Status::OK; }
    return NVIC_Status::NOK;
}
...
void NVIC::SetPriority(sint8 irqId, uint8 group, uint8 sub)
{
    ...
    if (irqId >= 0)
        NVIC_IPR(static_cast<uint8>(irqId)) = static_cast<uint8>((sub | (group << localVar)) << 4u);
}
